...
//- image
div(v-else-if="mode === 'image'" class="flex justify-center items-center bg-gray-200 rounded-lg border border-gray-400 shadow-md pt-2 pb-6 px-2 absolute bottom-3/4 max-w-screen-md w-screen")
  div(class="flex items-end h-72 w-full")
    ...
    div(v-if="isLoadingImages" class="bg-slate-50 flex justify-center items-center w-full h-72 overflow-y-scroll rounded-lg ml-2")
      div(role="status")
    ...
    div(v-else class="bg-slate-50 flex-1 grid grid-cols-3 h-72 w-full overflow-y-scroll rounded-lg ml-2")
      //- image list
      div(v-for="image of uploadedImages" :key="image.id" class="relative flex justify-center items-center")
        ...
        img(:src="image.dataURL" class="w-full aspect-auto col-span-1 p-2 hover:cursor-grab active:cursor-grabbing" @dragstart="(e) => {setDragUrl(e);}")
</template>
