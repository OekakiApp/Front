...
const checkFontSize = (value: string) => {
  ...
  if (Number.isNaN(parsedFontSize) || parsedFontSize <= 1) {
    parsedFontSize = 1
  }
...
div(class="tooltip block mr-2" :data-tip="'Font size'")
  ...
  input(id="font-size" ref="selectedFontSize" v-model="fontSize" type="number" class="block p-1 w-full text-gray-900 bg-gray-50 rounded-lg border border-gray-300 text-sm focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" required max="100" min="1" @input="(e) => handleInputFontSize(e)")
</template>
